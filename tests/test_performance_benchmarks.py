...
class TestIndexingPerformance:
    ...
    def test_indexing_1k_files_baseline(self, benchmark_repo_1k):
        ...
        assert metrics.indexing_time > 0, "Should have recorded indexing time"
        assert metrics.index_size > 0, "Should have created index files"
    ...
    def test_indexing_10k_files_primary_backend(self, benchmark_repo_10k):
        ...
        assert duration < 300, f"Indexing took {duration:.2f}s, expected <300s for primary backends"
...
class TestQueryPerformance:
    ...
    def test_query_1k_files_baseline(self, benchmark_repo_1k, tmp_path):
        ...
        output_path, tracker = query_and_pack(
            repo_path=benchmark_repo_1k, query="test function class", config=config
        )
        ...
        assert metrics.files_selected > 0, "Should have selected files"
        assert metrics.query_time > 0, "Should have recorded query time"
    ...
    def test_query_10k_files_primary_backend(self, benchmark_repo_10k, tmp_path):
        ...
        config = Config()
        config.backends = {"vector_index": "auto", "graph": "auto", "embeddings": "local"}
        ...
        output_path, tracker = query_and_pack(
            repo_path=benchmark_repo_10k, query="test function class method", config=config
        )
        ...
        assert duration < 10, f"Query took {duration:.2f}s, expected <10s for primary backends"
    ...
    def test_query_10k_files_fallback_backend(self, benchmark_repo_10k, tmp_path):
        ...
        config = Config()
        config.backends = {"vector_index": "faiss", "graph": "networkx", "embeddings": "local"}
        index_repository(benchmark_repo_10k, config=config)
        ...
        assert duration < 15, f"Query took {duration:.2f}s, expected <15s for fallback backends"
...
class TestMemoryUsage:
    ...
    def test_memory_usage_tracked_during_query(self, benchmark_repo_1k, tmp_path):
        ...
        output_path, tracker = query_and_pack(
            repo_path=benchmark_repo_1k, query="test function", config=config
        )
