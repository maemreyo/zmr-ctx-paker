...
class TestConfigValidation:
    ...
    def test_validate_weights_valid(self):
        ...
        config_data = {"semantic_weight": 0.8, "pagerank_weight": 0.2}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_weights_out_of_range(self):
        ...
        config_data = {"semantic_weight": 1.5, "pagerank_weight": -0.2}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_weights_wrong_type(self):
        ...
        config_data = {"semantic_weight": "0.6", "pagerank_weight": "0.4"}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_patterns_wrong_type(self):
        ...
        config_data = {"include_patterns": "**/*.py", "exclude_patterns": 123}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_patterns_mixed_types(self):
        ...
        config_data = {"include_patterns": ["**/*.py", 123, "**/*.js", None]}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_backends_invalid_values(self):
        ...
        config_data = {
            "backends": {"vector_index": "invalid", "graph": "unknown", "embeddings": "bad"}
        }
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_embeddings_invalid_device(self):
        ...
        config_data = {"embeddings": {"device": "gpu"}}  # Invalid, should be "cpu" or "cuda"
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_embeddings_invalid_batch_size(self):
        ...
        config_data = {"embeddings": {"batch_size": -10}}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_performance_valid(self):
        ...
        config_data = {
            "performance": {"max_workers": 8, "cache_embeddings": False, "incremental_index": False}
        }
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_validate_performance_invalid_max_workers(self):
        ...
        config_data = {"performance": {"max_workers": -1}}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
...
class TestConfigEdgeCases:
    ...
    def test_load_with_extra_fields(self):
        ...
        config_data = {"format": "xml", "unknown_field": "value", "another_unknown": 123}
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
    ...
    def test_weight_sum_warning(self):
        ...
        config_data = {"semantic_weight": 0.5, "pagerank_weight": 0.3}  # Sum is 0.8, not 1.0
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(config_data, f)
            temp_path = f.name
