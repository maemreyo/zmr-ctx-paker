...
from context_packer import init_cli


def test_init_cli_main_sets_executable_and_runs_script(monkeypatch) -> None:
    fake_file = "/repo/src/context_packer/init_cli.py"
    ...
    monkeypatch.setattr(init_cli.sys, "argv", ["ctx-packer-init", "--yes"])
...
def test_init_cli_main_exits_with_subprocess_return_code(monkeypatch) -> None:
    monkeypatch.setattr(init_cli.os.path, "abspath", lambda _: "/repo/src/context_packer/init_cli.py")
    monkeypatch.setattr(init_cli.os.path, "dirname", lambda _: "/repo/src/context_packer")
    monkeypatch.setattr(init_cli.os, "stat", lambda _: type("_S", (), {"st_mode": 0o755})())
    monkeypatch.setattr(init_cli.os, "chmod", lambda *_: None)
    monkeypatch.setattr(init_cli.os, "getcwd", lambda: "/repo")
    monkeypatch.setattr(init_cli.sys, "argv", ["ctx-packer-init"])
