...
class TestRetrievalEngine:
    ...
    def test_initialization_with_default_weights(self):
        ...
        engine = RetrievalEngine(vector_index=vector_index, graph=graph)
    ...
    def test_initialization_with_custom_weights(self):
        ...
        engine = RetrievalEngine(
            vector_index=vector_index, graph=graph, semantic_weight=0.7, pagerank_weight=0.3
        )
    ...
    def test_initialization_with_invalid_weights(self):
        ...
        with pytest.raises(ValueError, match="must sum to 1.0"):
            RetrievalEngine(
                vector_index=vector_index, graph=graph, semantic_weight=0.5, pagerank_weight=0.6
            )

        # Negative weight
        with pytest.raises(ValueError, match="must be in"):
            RetrievalEngine(
                vector_index=vector_index, graph=graph, semantic_weight=-0.1, pagerank_weight=1.1
            )

        # Weight > 1.0
        with pytest.raises(ValueError, match="must be in"):
            RetrievalEngine(
                vector_index=vector_index, graph=graph, semantic_weight=1.5, pagerank_weight=-0.5
            )
    ...
    def test_normalize_scores_to_zero_one_range(self):
        ...
        scores = {"a.py": 0.5, "b.py": 1.0, "c.py": 0.0, "d.py": 0.75}

        normalized = engine._normalize(scores)

        # Check all values are in [0, 1]
        for _file, score in normalized.items():
    ...
    def test_normalize_equal_scores(self):
        ...
        scores = {"a.py": 0.5, "b.py": 0.5, "c.py": 0.5}
    ...
    def test_merge_scores_with_correct_weights(self):
        ...
        vector_index = MockVectorIndex()
        graph = MockRepoMapGraph()
        engine = RetrievalEngine(vector_index, graph, semantic_weight=0.6, pagerank_weight=0.4)

        semantic_scores = {"a.py": 1.0, "b.py": 0.5}

        pagerank_scores = {"a.py": 0.5, "c.py": 1.0}
    ...
    def test_merge_scores_with_equal_weights(self):
        ...
        vector_index = MockVectorIndex()
        graph = MockRepoMapGraph()
        engine = RetrievalEngine(vector_index, graph, semantic_weight=0.5, pagerank_weight=0.5)
    ...
    def test_handling_missing_semantic_scores(self):
        ...
        graph.set_pagerank_scores({"a.py": 0.5, "b.py": 0.3, "c.py": 0.2})
    ...
    def test_handling_missing_pagerank_scores(self):
        ...
        vector_index.set_search_results({"a.py": 0.9, "b.py": 0.7, "c.py": 0.5})
    ...
    def test_retrieve_returns_sorted_results(self):
        ...
        # Set mock data
        vector_index.set_search_results({"a.py": 0.5, "b.py": 0.8, "c.py": 0.3})

        graph.set_pagerank_scores({"a.py": 0.7, "b.py": 0.2, "c.py": 0.9})
    ...
    def test_retrieve_respects_top_k(self):
        ...
        vector_index.set_search_results({f"file_{i}.py": 0.5 + i * 0.1 for i in range(5)})

        graph.set_pagerank_scores({f"file_{i}.py": 0.2 for i in range(5)})
    ...
    def test_retrieve_with_changed_files(self):
        ...
        # Set mock data
        vector_index.set_search_results({"a.py": 0.5, "b.py": 0.5})

        graph.set_pagerank_scores({"a.py": 0.3, "b.py": 0.7})  # Higher PageRank (simulating boost)

        engine = RetrievalEngine(vector_index, graph, semantic_weight=0.5, pagerank_weight=0.5)

        results = engine.retrieve(query="test", changed_files=["b.py"], top_k=10)
