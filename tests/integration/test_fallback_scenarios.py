    ...
    def test_xml_output_workflow(self, small_repo, tmp_path):
        ...
        **Validates: Requirements 15.2, 6.7**
        ...
        with patch("sentence_transformers.SentenceTransformer") as mock_st:
            ...
            # Index repository
            index_repository(repo_path=str(small_repo), config=config)

            # Query and pack with XML format
            output_path, _ = query_and_pack(
                repo_path=str(small_repo), query="authentication logic", config=config
            )
    ...
    def test_zip_output_workflow(self, small_repo, tmp_path):
        ...
        **Validates: Requirements 15.2, 7.1, 7.2, 7.3**
        ...
        with zipfile.ZipFile(output_path, "r") as zip_file:
            ...
            file_entries = [
                f for f in zip_contents if f.startswith("files/") and not f.endswith("/")
            ]
            assert len(file_entries) > 0, "ZIP must contain files under files/ directory"
        ...
        **Validates: Requirements 15.3, 10.7**
        ...
        with patch("sentence_transformers.SentenceTransformer") as mock_st:
            ...
            with patch("ws_ctx_engine.vector_index.LEANNIndex", create=True) as mock_leann:
                ...
                output_path, _ = query_and_pack(
                    repo_path=str(small_repo), query="authentication", config=config
                )
