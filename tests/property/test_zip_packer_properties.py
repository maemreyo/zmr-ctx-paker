...
def metadata_strategy(draw):
    ...
    if draw(st.booleans()):
        changed_files = draw(st.lists(file_name_strategy(), max_size=5))
        if changed_files:
    ...
    def test_property_zip_structure_preservation(self, file_paths, metadata):
        ...
        **Validates: Requirements 7.1, 7.2, 7.3**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            importance_scores = dict.fromkeys(file_paths, 0.5)
            ...
            # Verify ZIP is valid
            assert isinstance(zip_bytes, bytes), "pack() must return bytes"
            assert len(zip_bytes) > 0, "ZIP archive must not be empty"
            ...
            with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zip_file:
                ...
                assert (
                    "REVIEW_CONTEXT.md" in zip_contents
                ...
                for file_path in file_paths:
                    ...
                    matching_paths = [
                        zp for zp in zip_contents if zp.replace("\\", "/") == expected_zip_path
                    ]
        ...
        **Validates: Requirements 7.4, 7.5, 7.6**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zip_file:
                ...
                assert (
                    metadata["repo_name"] in manifest_content
                ), "Manifest must include repository name"
                assert (
                    str(metadata.get("file_count", len(file_paths))) in manifest_content
                ), "Manifest must include file count"
                assert (
                    str(metadata.get("total_tokens", 0)) in manifest_content
                    or f"{metadata.get('total_tokens', 0):,}" in manifest_content
                ), "Manifest must include total tokens"

                # Verify all files are listed with scores
                for file_path, score in importance_scores.items():
                    assert file_path in manifest_content, f"Manifest must list file '{file_path}'"

                    # Score should appear in manifest (formatted to 4 decimal places)
                    score_str = f"{score:.4f}"
                    assert (
                        score_str in manifest_content
                ...
                assert (
                    "Suggested Reading Order" in manifest_content
                    or "Reading Order" in manifest_content
                ...
                if len(sorted_files) >= 2:
                    ...
                    manifest_content.find(top_file)
                    manifest_content.find(second_file)
                    ...
                    reading_order_section = (
                        manifest_content.split("Suggested Reading Order")[-1]
                        if "Suggested Reading Order" in manifest_content
                        else manifest_content.split("Reading Order")[-1]
                    )
                    ...
                    if top_pos_in_order >= 0 and second_pos_in_order >= 0:
                        assert (
                            top_pos_in_order < second_pos_in_order
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zip_file:
                for file_path, original_content in file_contents.items():
                    ...
                    matching_paths = [
                        zp for zp in zip_file.namelist() if zp.replace("\\", "/") == zip_path
                    ]

                    assert len(matching_paths) > 0, f"File '{file_path}' not found in ZIP"
    ...
    def test_property_zip_handles_empty_file_list(self, metadata):
        ...
        **Validates: Requirements 7.1, 7.3**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify ZIP is valid
            assert isinstance(zip_bytes, bytes), "pack() must return bytes even for empty file list"
            assert len(zip_bytes) > 0, "ZIP archive must not be empty even with no files"
            ...
            with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zip_file:
                ...
                manifest_content = zip_file.read("REVIEW_CONTEXT.md").decode("utf-8")
                assert len(manifest_content) > 0, "Manifest must not be empty"
                assert (
                    metadata["repo_name"] in manifest_content
