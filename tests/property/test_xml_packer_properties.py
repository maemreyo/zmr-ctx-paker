...
def metadata_strategy(draw):
    ...
    metadata = {"repo_name": repo_name, "file_count": file_count, "total_tokens": total_tokens}
    ...
    if draw(st.booleans()):
        changed_files = draw(st.lists(file_name_strategy(), max_size=10))
    ...
    def test_property_xml_generation_completeness(self, metadata, file_names):
        ...
        **Validates: Requirements 6.1, 6.2, 6.3, 6.5**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify root element
            assert root.tag == "repository", f"Root element must be 'repository', got '{root.tag}'"

            # Verify metadata section exists
            metadata_elem = root.find("metadata")
            assert metadata_elem is not None, "XML must contain <metadata> element"

            # Verify metadata fields
            name_elem = metadata_elem.find("name")
            assert name_elem is not None, "Metadata must contain <name> element"
            assert name_elem.text == str(
                metadata["repo_name"]
            ), f"Name mismatch: expected '{metadata['repo_name']}', got '{name_elem.text}'"

            file_count_elem = metadata_elem.find("file_count")
            assert file_count_elem is not None, "Metadata must contain <file_count> element"
            assert file_count_elem.text == str(
                metadata["file_count"]
            ...
            assert total_tokens_elem is not None, "Metadata must contain <total_tokens> element"
            assert total_tokens_elem.text == str(
            ...
            assert files_elem is not None, "XML must contain <files> element"

            # Verify all files are present
            file_elems = files_elem.findall("file")
            assert len(file_elems) == len(
                file_paths
            ), f"Expected {len(file_paths)} file elements, got {len(file_elems)}"

            # Verify each file element has required attributes
            for file_elem in file_elems:
                assert "path" in file_elem.attrib, "File element must have 'path' attribute"
                assert "tokens" in file_elem.attrib, "File element must have 'tokens' attribute"

                # Verify tokens is a valid integer
                try:
                    tokens = int(file_elem.attrib["tokens"])
                    assert tokens >= 0, f"Token count must be non-negative, got {tokens}"
                except ValueError:
                    pytest.fail(
                        f"Token count must be an integer, got '{file_elem.attrib['tokens']}'"
                    )
    ...
    def test_property_xml_character_escaping(self, content):
        ...
        **Validates: Requirements 6.4**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Pack file
            packer = XMLPacker()
            metadata = {"repo_name": "test-repo", "file_count": 1, "total_tokens": 100}
    ...
    def test_property_xml_round_trip_validity(self, metadata, file_names):
        ...
        **Validates: Requirements 6.7**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert root2.tag == "repository", "Root element lost after round-trip"

            metadata_elem = root2.find("metadata")
            assert metadata_elem is not None, "Metadata element lost after round-trip"

            files_elem = root2.find("files")
            assert files_elem is not None, "Files element lost after round-trip"
    ...
    def test_property_xml_handles_empty_file_list(self, file_names):
        ...
        **Validates: Requirements 6.1, 6.3**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            packer = XMLPacker()
            metadata = {"repo_name": "test-repo", "file_count": 0, "total_tokens": 0}
            ...
            # Verify structure
            files_elem = root.find("files")
            assert files_elem is not None, "Files element must exist even when empty"

            file_elems = files_elem.findall("file")
            assert len(file_elems) == 0, f"Expected 0 file elements, got {len(file_elems)}"
        ...
        **Validates: Requirements 6.5**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for file_elem in files_elem.findall("file"):
                ...
                assert tokens_str is not None, "File element must have tokens attribute"
