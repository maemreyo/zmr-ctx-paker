...
def test_property_score_merging_correctness(semantic_scores, pagerank_scores, weights):
    ...
    for file in all_files:
        ...
        expected_score = semantic_weight * semantic_score + pagerank_weight * pagerank_score

        actual_score = merged[file]

        # Allow small floating-point error
        assert (
            abs(actual_score - expected_score) < 1e-9
...
def test_property_score_normalization_range(scores):
    ...
    for file, score in normalized.items():
        assert 0.0 <= score <= 1.0, f"Normalized score for {file} is out of range: {score}"
    ...
    if len(normalized) > 0:
        ...
        if len(set(values)) > 1:
            ...
            assert all(
                abs(v - 1.0) < 1e-9 for v in values
...
def test_property_retrieval_output_format(semantic_scores, pagerank_scores, top_k):
    ...
    # Retrieve with query
    results = engine.retrieve(query="test query", changed_files=None, top_k=top_k)
    ...
    assert len(results) <= top_k, f"Results length {len(results)} exceeds top_k {top_k}"

    # Verify all scores are in valid range [0, 1]
    for file_path, score in results:
        assert 0.0 <= score <= 1.0, f"Score for {file_path} is out of range: {score}"
...
def test_property_weights_validation(semantic_weight, pagerank_weight):
    ...
    weights_valid = (
        0 <= semantic_weight <= 1
        and 0 <= pagerank_weight <= 1
        and abs(semantic_weight + pagerank_weight - 1.0) < 0.001
    )
