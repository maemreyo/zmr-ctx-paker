        ...
        for i, (filename, score) in enumerate(ranked_files):
            if filename in selected_set:
                ...
                for j in range(i):
                    ...
                    assert prev_filename in selected_set, \
...
def test_budget_enforcement(file_data, token_budget):
    ...
    try:
        ...
        assert manager.content_budget == content_budget, \
...
def test_budget_manager_output_completeness(file_data, token_budget):
    ...
    try:
        ...
        assert isinstance(total_tokens, int), f"Total tokens should be int, got {type(total_tokens)}"
    ...
    try:
        ...
        for selected_file in selected:
            for unselected_file in unselected:
                ...
                new_tokens = total_tokens - file_info[selected_file][2] + file_info[unselected_file][2]
                
                # If swap stays within budget
                if new_tokens <= manager.content_budget:
                    # Calculate new importance score
                    new_score = current_score - file_info[selected_file][1] + file_info[unselected_file][1]
