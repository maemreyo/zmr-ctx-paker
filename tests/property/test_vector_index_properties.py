...
class TestVectorIndexSearchOrdering:
    ...
    def test_leann_search_ordering(self, chunks, query):
        ...
        num_files = len({chunk.path for chunk in chunks})
        ...
        with patch.object(index, "_embedding_generator", None):
            ...
            assert all(
                -1.0 <= score <= 1.0 for score in scores
...
class TestEmbeddingFallbackChain:
    ...
    def test_embedding_fallback_on_oom(self, texts):
        ...
        with (
            patch.dict(
                "sys.modules", {"sentence_transformers": mock_st_module, "openai": mock_openai}
            ),
            ...
            try:
                ...
                assert "Failed to generate embeddings" in str(
                    e
                ) or "Failed to initialize API client" in str(e)
    ...
    def test_embedding_fallback_on_low_memory(self, texts):
        ...
        with patch("ws_ctx_engine.vector_index.vector_index.psutil.virtual_memory") as mock_mem:
            ...
            with pytest.raises(
                RuntimeError, match="Failed to generate embeddings|Failed to initialize API client"
            ):
                gen.encode(texts)
...
class TestVectorIndexRoundTrip:
    ...
    def test_leann_save_load_preserves_search(self, chunks, query, seed):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for (path1, score1), (path2, score2) in zip(
