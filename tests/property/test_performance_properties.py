...
from ws_ctx_engine.monitoring import PerformanceMetrics, PerformanceTracker
...
"""
        ...
        assert metrics.indexing_time > 0, "Indexing time must be reported and positive"

        assert metrics.files_processed > 0, "Files processed must be reported and positive"

        assert metrics.index_size > 0, "Index size must be reported and positive"

        # Verify metrics are accessible via to_dict
        metrics_dict = metrics.to_dict()
        assert "indexing_time" in metrics_dict, "indexing_time must be in metrics dictionary"
        assert "files_processed" in metrics_dict, "files_processed must be in metrics dictionary"
        assert "index_size" in metrics_dict, "index_size must be in metrics dictionary"
...
"""
        ...
        output_path, meta = query_and_pack(
            repo_path=str(repo_path), query="test function", config=config
        )
        ...
        assert metrics.query_time > 0, "Query time must be reported and positive"

        assert metrics.files_selected > 0, "Files selected must be reported and positive"

        assert metrics.total_tokens > 0, "Total tokens must be reported and positive"

        # Verify metrics are accessible via to_dict
        metrics_dict = metrics.to_dict()
        assert "query_time" in metrics_dict, "query_time must be in metrics dictionary"
        assert "files_selected" in metrics_dict, "files_selected must be in metrics dictionary"
        assert "total_tokens" in metrics_dict, "total_tokens must be in metrics dictionary"
    ...
    def test_property_memory_tracking_reported(self, tmp_path):
        ...
        **Validates: Requirements 13.3**
        ...
        assert metrics.memory_usage >= 0, "Memory usage must be reported and non-negative"

        # Verify it's in the metrics dictionary
        metrics_dict = metrics.to_dict()
        assert "memory_usage" in metrics_dict, "memory_usage must be in metrics dictionary"
    ...
    def test_property_phase_timings_reported(self, tmp_path):
        ...
        **Validates: Requirements 13.1, 13.2**
        ...
        assert isinstance(metrics.phase_timings, dict), "Phase timings must be a dictionary"

        # Should have at least some phases tracked
        assert len(metrics.phase_timings) > 0, "At least one phase timing should be tracked"

        # All phase timings should be positive
        for phase, duration in metrics.phase_timings.items():
            assert duration > 0, f"Phase {phase} duration must be positive, got {duration}"
    ...
    def test_property_metrics_format_readable(self, tmp_path):
        ...
        **Validates: Requirements 13.1, 13.2**
        ...
        assert isinstance(formatted, str), "Formatted metrics must be a string"
        assert len(formatted) > 0, "Formatted metrics must not be empty"

        # Should contain key information
        assert (
            "Indexing Metrics" in formatted or "indexing" in formatted.lower()
    ...
    def test_property_metrics_to_dict_completeness(self):
        ...
        **Validates: Requirements 13.1, 13.2, 13.3**
        ...
        for field in required_fields:
            assert field in metrics_dict, f"Field {field} must be in metrics dictionary"
