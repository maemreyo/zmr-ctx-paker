...
from ws_ctx_engine.logger import WsCtxEngineLogger
...
def test_property_34_comprehensive_error_logging_fallback(component, primary, fallback, reason):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        logger.log_fallback(component=component, primary=primary, fallback=fallback, reason=reason)
...
def test_property_34_error_with_context():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        context = {"file_path": "src/main.py", "line_number": 42, "function": "test_function"}
...
@given(backend=st.text(min_size=1, max_size=20), install_cmd=st.text(min_size=1, max_size=50))
...
def test_property_37_verbose_mode_timing_with_metrics(
    phase, duration, files_processed, tokens_counted
):
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        logger.log_phase(
            phase, duration, files_processed=files_processed, tokens_counted=tokens_counted
        )
