...
def test_property_22_configuration_loading(config_data):
    ...
    try:
        ...
        assert (
            config.performance["cache_embeddings"] == config_data["performance"]["cache_embeddings"]
        )
        assert (
            config.performance["incremental_index"]
            == config_data["performance"]["incremental_index"]
        )
...
@given(
    invalid_format=st.one_of(
        ...
        st.floats(),
        st.lists(st.text()),
        st.text().filter(lambda x: x.lower() not in ["xml", "zip"]),
    ),
