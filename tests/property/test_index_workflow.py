...
@settings(
    max_examples=5, deadline=60000, suppress_health_check=[HealthCheck.function_scoped_fixture]
...
def test_property_25_backend_auto_detection(temp_repo):
    ...
    assert vector_backend in valid_vector_backends, f"Invalid vector backend: {vector_backend}"
    assert graph_backend in valid_graph_backends, f"Invalid graph backend: {graph_backend}"
    ...
    assert graph.__class__.__name__ == graph_backend, (
        f"Loaded graph type {graph.__class__.__name__} " f"doesn't match metadata {graph_backend}"
    )
...
def test_property_26_index_staleness_detection(temp_repo):
    ...
    assert not metadata.is_stale(temp_repo), "Freshly built indexes should not be stale"
    ...
    assert metadata.is_stale(temp_repo), "Indexes should be stale after file modification"

    # Test with deleted file
    utils_py = Path(temp_repo) / "utils.py"
    utils_py.unlink()

    assert metadata.is_stale(temp_repo), "Indexes should be stale after file deletion"
...
def test_property_27_automatic_index_rebuild(temp_repo):
    ...
    assert (
        metadata.created_at > original_timestamp
    ), "Indexes should have been rebuilt with new timestamp"

    # Verify indexes are no longer stale
    assert not metadata.is_stale(temp_repo), "Rebuilt indexes should not be stale"
    ...
    assert (
        metadata2.created_at == metadata.created_at
    ), "Indexes should not be rebuilt when auto_rebuild=False"

    # But they should still be stale
    assert metadata2.is_stale(temp_repo), "Indexes should be stale when auto_rebuild=False"
...
def test_index_workflow_integration(temp_repo):
    ...
    config = Config(token_budget=50000, semantic_weight=0.6, pagerank_weight=0.4)
