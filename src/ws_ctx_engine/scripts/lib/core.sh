...
_render_template() {
  ...
  CTX_ENGINE_VERSION="$(wsctx --version 2>/dev/null | head -1 || echo 'latest')"
  CTX_DATE="$(date +%Y-%m-%d)"
  CTX_TARGET_NAME="$(basename "$CTX_TARGET")"
  
  export CTX_ENGINE_VERSION
  export CTX_DATE
...
Options:
  ...
  --skip-index      Skip running wsctx index after setup
...
show_version() {
  ...
  if command -v wsctx &> /dev/null; then
    local version
    version=$(wsctx --version 2>/dev/null || echo "wsctx version: unknown")
    echo "$version"
  else
    echo "wsctx-init (wsctx not installed yet)"
  fi
}
