...
class RegexChunker(ASTChunker):
    ...
    _PATTERNS = {
        ...
        "rust": {
            ...
            "impl": re.compile(
                r"^\s*impl(?:<[^>]+>)?\s+(?:[^for]+for\s+)?(\w+)\s*[{]", re.MULTILINE
            ),
    ...
    def _parse_file(self, file_path: Path, repo_root: Path) -> list[CodeChunk]:
        ...
        for _pattern_name, pattern in self._PATTERNS[language].items():
    ...
    def _find_block_end(
        self, content: str, definition_start: int, lines: list[str], language: str
    ) -> int:
    ...
    def _brace_matching_end(
        self, content: str, definition_start: int, lines: list[str], start_line_0: int
    ) -> int:
