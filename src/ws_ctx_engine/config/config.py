...
class Config:
    ...
    def _validate_format(value: Any, logger: Any) -> str:
        ...
        if value not in ["xml", "zip", "json", "yaml", "md", "toon"]:
            logger.error(
                f"Invalid format: {value}, must be 'xml', 'zip', 'json', 'yaml', 'md', or 'toon'"
            )
            return "zip"
    ...
    def _validate_backends(value: Any, logger: Any) -> dict[str, str]:
        ...
        if not isinstance(value, dict):
            ...
            return {"vector_index": "auto", "graph": "auto", "embeddings": "auto"}
    ...
    def _validate_performance(value: Any, logger: Any) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            ...
            return {"max_workers": 4, "cache_embeddings": True, "incremental_index": True}

        performance = {"max_workers": 4, "cache_embeddings": True, "incremental_index": True}
        ...
        if "max_workers" in value:
            ...
            if isinstance(max_workers, int) and max_workers > 0:
                ...
                logger.error(
                    f"Invalid performance.max_workers: {max_workers}, must be positive int"
                )
