...
class IGraphRepoMap(RepoMapGraph):
    ...
    def __init__(self, boost_factor: float = 2.0):
        ...
        try:
            ...
            raise ImportError(
                "python-igraph is not installed. " "Install with: pip install python-igraph"
            ) from e
    ...
    def pagerank(self, changed_files: list[str] | None = None) -> dict[str, float]:
        ...
        try:
            ...
            pagerank_scores = {self.vertex_to_file[idx]: score for idx, score in enumerate(scores)}
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            raise OSError(f"Failed to save graph to {path}: {e}") from e
...
class NetworkXRepoMap(RepoMapGraph):
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            # Save graph data
            data = {"backend": "networkx", "boost_factor": self.boost_factor, "graph": self.graph}
...
def create_graph(backend: str = "auto", boost_factor: float = 2.0) -> RepoMapGraph:
    ...
    if backend in ("auto", "igraph"):
        try:
            graph: RepoMapGraph = IGraphRepoMap(boost_factor=boost_factor)
            ...
            if backend == "igraph":
                ...
                logger.log_fallback(
                    component="graph", primary="igraph", fallback="networkx", reason=str(e)
                )
    ...
    try:
        ...
        raise ImportError(
            "NetworkX backend not available. " "Install with: pip install networkx"
        ) from e
