...
def index(
    ...
    try:
        ...
        _emit_ndjson({
            ...
            "index_dir": str(repo_path_obj / '.context-pack'),
...
def search(
    ...
    try:
        ...
        console.print("\n[yellow]Suggestion:[/yellow] Run 'ctx-packer index' first to build indexes")
...
def mcp(
    ...
    mcp_config: Optional[str] = typer.Option(
        None,
        "--mcp-config",
        help="Path to MCP config JSON (defaults to .context-pack/mcp_config.json)",
    ),
    ...
    """Run ctx-packer as an MCP stdio server bound to a single workspace."""
    try:
