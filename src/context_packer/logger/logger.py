"""
Logging infrastructure for Context Packer.
...
class ContextPackerLogger:
    ...
    def __init__(self, log_dir: str = ".context-pack/logs", name: str = "context_packer"):
        """
        Initialize logger with dual output handlers.
        
        Args:
            log_dir: Directory for log files (default: .context-pack/logs)
            name: Logger name (default: context_packer)
        """
        ...
        log_filename = f"context-packer-{datetime.now():%Y%m%d-%H%M%S}.log"
    ...
    def log_phase(self, phase: str, duration: float, **metrics: Any) -> None:
        ...
        if metrics_str:
            ...
            self.logger.info(
                f"Phase complete | phase={phase} | duration={duration:.2f}s"
            )
...
_global_logger: Optional[ContextPackerLogger] = None


def get_logger(log_dir: str = ".context-pack/logs") -> ContextPackerLogger:
    """
    ...
    Returns:
        Global ContextPackerLogger instance
    """
    global _global_logger
    if _global_logger is None:
        _global_logger = ContextPackerLogger(log_dir=log_dir)
