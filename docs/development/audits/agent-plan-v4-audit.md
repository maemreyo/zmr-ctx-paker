# ws-ctx-engine Agent Plan v4 — Implementation Audit Report

_Date:_ 2026-03-25  
_Scope:_ Audit of `docs/for-agents/ws-ctx-engine-agent-plan-v4.md` against the current repository state.
...
Plan source: `docs/for-agents/ws-ctx-engine-agent-plan-v4.md:716-734`

| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 1.1 | Implement `ws-ctx-engine search "<query>"` | Pass | `src/ws_ctx_engine/cli/cli.py:189-291`, `src/ws_ctx_engine/workflow/query.py:151-215` | Implemented as a first-class CLI command. |
| 1.2 | Add global `--agent-mode` flag (all commands) | Partial | `src/ws_ctx_engine/cli/cli.py:88-107`, `src/ws_ctx_engine/cli/cli.py:435-455`, `src/ws_ctx_engine/cli/cli.py:560-597`, `src/ws_ctx_engine/cli/cli.py:655-685` | Global flag exists, but `query`, `pack`, and `status` still emit human-readable output instead of fully clean NDJSON behavior. |
| 1.3 | Add `--limit` flag to `search` | Pass | `src/ws_ctx_engine/cli/cli.py:201-208`, `src/ws_ctx_engine/workflow/query.py:168-169` | Matches default and bounds from the plan. |
| 1.4 | Add `domain_filter` option to `search` | Pass | `src/ws_ctx_engine/cli/cli.py:209-213`, `src/ws_ctx_engine/workflow/query.py:195-210` | Implemented as `--domain-filter`. |
| 1.5 | Update `SKILL.md` template to teach `search` not `pack` | Partial | `src/ws_ctx_engine/templates/claude/SKILL.md.tpl:16-19`, `src/ws_ctx_engine/scripts/lib/commands.sh:12-18,27-34` | Template exists, but still points to `CTX_CMD_QUERY`, and canonical command export still defines `query` instead of `search`. |
| 1.6 | Update `.cursorrules` template | Fail | `src/ws_ctx_engine/templates/cursor/ws-ctx-engine.mdc.tpl:12-17,35`, `src/ws_ctx_engine/scripts/lib/commands.sh:27-34` | No `.cursorrules` template was found; current Cursor rule template exists in `.mdc` form but still relies on `CTX_CMD_QUERY`. |
| 1.7 | Update `--help` output for agent-mode | Pass | `src/ws_ctx_engine/cli/cli.py:98-101` | Help text is present. |
| 1.D | Deliverable: `ws-ctx-engine search "query" --agent-mode` returns clean NDJSON | Partial | `src/ws_ctx_engine/cli/cli.py:261-281`, `tests/unit/test_cli.py:427-446` | Search NDJSON exists, but tests only verify top-level `--agent-mode search ...`; the exact invocation form shown in the plan is not validated. |
...
Plan source: `docs/for-agents/ws-ctx-engine-agent-plan-v4.md:742-763`

| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 2.1 | Implement `--format json` for `pack` | Pass | `src/ws_ctx_engine/cli/cli.py:491-496,534-538`, `src/ws_ctx_engine/workflow/query.py:434-465`, `src/ws_ctx_engine/output/json_formatter.py:9-15` | Implemented and tested. |
| 2.2 | Implement `--format md` for `pack` | Pass | `src/ws_ctx_engine/cli/cli.py:491-496,534-538`, `src/ws_ctx_engine/workflow/query.py:434-469`, `src/ws_ctx_engine/output/md_formatter.py:29-76` | Implemented and includes data-boundary comments. |
| 2.3 | Implement CLI `--secrets-scan` | Partial | `src/ws_ctx_engine/cli/cli.py:394-398`, `src/ws_ctx_engine/cli/cli.py:515-518`, `src/ws_ctx_engine/workflow/query.py:444-457` | Flag exists and works for JSON/MD payload building, but not for XML/ZIP pack paths. |
| 2.4 | Build secret scan cache (`secret_scan_cache.json`) | Pass | `src/ws_ctx_engine/secret_scanner.py:29-33,42-65,78-81` | Implemented exactly with `mtime` + `inode` keyed persistence. |
| 2.5 | Integrate `secretlint` as Python subprocess with regex fallback | Fail | `src/ws_ctx_engine/secret_scanner.py:14-20,58-66` | Current implementation is regex-only; no `secretlint` subprocess integration was found in repo code. |
| 2.6 | Add `index_health` object in all responses | Partial | `src/ws_ctx_engine/workflow/query.py:390-401`, `src/ws_ctx_engine/output/json_formatter.py:9-15`, `src/ws_ctx_engine/output/md_formatter.py:29-76`, `src/ws_ctx_engine/packer/xml_packer.py:114-138`, `src/ws_ctx_engine/packer/zip_packer.py:153-167` | Present in JSON metadata and MCP/search responses, but not rendered in Markdown/XML/ZIP output. |
| 2.D | Deliverable: `ws-ctx-engine pack --format json --secrets-scan` produces clean, parseable JSON | Partial | `src/ws_ctx_engine/workflow/query.py:462-472`, `src/ws_ctx_engine/cli/cli.py:581-597` | JSON payload is produced, but written to a file rather than emitted as parseable stdout as implied by the acceptance test. |
| 2.A | Acceptance test: pipe `ws-ctx-engine pack --query "auth" --format json` into `python3 -c` | Fail | `src/ws_ctx_engine/cli/cli.py:588-597` | Current CLI prints success/status and saves the JSON to disk; it does not stream the JSON payload to stdout in that form. |
...
Plan source: `docs/for-agents/ws-ctx-engine-agent-plan-v4.md:771-819`

### Week 7–8: Core MCP Infrastructure

| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 3.1 | Create `ws_ctx_engine/mcp_server.py` | Pass | `src/ws_ctx_engine/mcp_server.py:1-9` | File exists and forwards to the MCP server runtime. |
| 3.2 | Implement `ws-ctx-engine mcp --workspace PATH` entry point | Pass | `src/ws_ctx_engine/cli/cli.py:307-347` | Implemented. |
| 3.3 | Implement workspace-binding / scope isolation | Pass | `src/ws_ctx_engine/mcp/server.py:13-30`, `src/ws_ctx_engine/mcp/tools.py:25-35`, `src/ws_ctx_engine/mcp/config.py:100-111` | Implemented. |
| 3.4 | Implement Layer 3 path traversal protection | Pass | `src/ws_ctx_engine/mcp/security/path_guard.py:6-30`, `src/ws_ctx_engine/mcp/tools.py:158-180` | Implemented and exercised by tests. |
| 3.5 | Generate `SESSION_TOKEN` at startup | Pass | `src/ws_ctx_engine/mcp/security/rade_delimiter.py:6-17`, `src/ws_ctx_engine/mcp/tools.py:31-33` | Implemented through `RADESession`. |
| 3.6 | Implement Layer 1 read-only enforcement | Pass | `src/ws_ctx_engine/mcp/tools.py:37-82` | Only 4 read-only tools are exposed; unknown tools are rejected. |

### Week 9: Tool Implementation

| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 3.7 | Implement `search_codebase` tool | Pass | `src/ws_ctx_engine/mcp/tools.py:113-140` | Implemented. |
| 3.8 | Implement `get_file_context` tool | Pass | `src/ws_ctx_engine/mcp/tools.py:142-221` | Implemented. |
| 3.9 | Implement `get_domain_map` tool | Pass | `src/ws_ctx_engine/mcp/tools.py:223-279` | Implemented with cache support. |
| 3.10 | Implement `get_index_status` tool | Pass | `src/ws_ctx_engine/mcp/tools.py:280-299` | Implemented. |
| 3.11 | Implement `index_health` computation for all tools | Pass | `src/ws_ctx_engine/mcp/tools.py:126-136,156-168,274-277,289-299` | Implemented across MCP responses. |

### Week 10: Security Layer

| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 3.12 | Integrate secret scan cache into `get_file_context` | Pass | `src/ws_ctx_engine/mcp/tools.py:183-195`, `src/ws_ctx_engine/secret_scanner.py:29-81` | Always-on in MCP path. |
| 3.13 | RADE delimiter wrapping in all file reads | Pass | `src/ws_ctx_engine/mcp/security/rade_delimiter.py:6-22`, `src/ws_ctx_engine/mcp/tools.py:210-220` | Implemented. |
| 3.14 | Rate limiting middleware — token bucket, configurable | Partial | `src/ws_ctx_engine/mcp/security/rate_limiter.py:7-29`, `src/ws_ctx_engine/mcp/config.py:9-14,59-80` | Configurable limits exist, but implementation is a sliding-window counter, not a token bucket. |
...
| # | Plan item | Status | Evidence | Notes |
|---|---|---|---|---|
| 3.16 | `.ws-ctx-engine/mcp_config.json` schema | Pass | `src/ws_ctx_engine/mcp/config.py:21-111`, `src/ws_ctx_engine/templates/mcp/mcp_config.json.tpl:1-10` | Implemented. |
...
| 3.20 | `wsctx-init` emits MCP config | Pass | `src/ws_ctx_engine/scripts/init.sh:72-76`, `src/ws_ctx_engine/scripts/lib/mcp.sh:4-15` | Implemented. |
...
| 3.D | Deliverable: running MCP server usable via `mcpServers` config | Pass | `src/ws_ctx_engine/cli/cli.py:307-347`, `src/ws_ctx_engine/mcp/server.py:13-48` | Core runtime is present. |
...
Evidence:
- `docs/for-agents/ws-ctx-engine-agent-plan-v4.md:249`
- `docs/for-agents/ws-ctx-engine-agent-plan-v4.md:629-633`
- `src/ws_ctx_engine/mcp/security/rade_delimiter.py:6-22`
- `src/ws_ctx_engine/mcp/tools.py:210-220`
...
11. **Add command-shape tests for the exact CLI invocations shown in the plan**
   - Especially `ws-ctx-engine search "..." --agent-mode --limit 3`
