# ws-ctx-engine for AI Agents: Comprehensive Implementation Plan (v4)
...
4. [Solution 1 — Search-Only CLI (`ws-ctx-engine search`)](#4-solution-1--search-only-cli)
...
`ws-ctx-engine` was designed as a *Human-to-AI* bridge: a developer runs the CLI, exports a ZIP or XML, and uploads it to an LLM context window. With the rise of autonomous coding agents (Claude Code, TRAE, SWE-agent), the paradigm has shifted to *AI-to-Codebase* interactions. The current tool has three critical gaps:
...
| **No MCP support** | Agents cannot discover or use ws-ctx-engine capabilities natively |
...
**The true differentiation of ws-ctx-engine is the retrieval engine**, which no competitor has:

| Capability | ws-ctx-engine | repomix | aider |
|---|---|---|---|
...
**Strategic conclusion:** The goal of this project is not to replicate repomix. It is to *expose ws-ctx-engine's superior retrieval engine via MCP*. An agent using ws-ctx-engine's `search_codebase` gets semantically ranked results with PageRank-weighted dependency context — something repomix cannot provide. That is the moat.
...
└────────────────────┬───────────────────────┬────────────────┘
          ...
          ┌──────────▼──────────┐   ┌────────▼────────┐
          │   ws-ctx-engine MCP    │   │  ws-ctx-engine CLI  │
          │      Server         │   │  search --agent  │
          └──────────┬──────────┘   └────────┬─────────┘
                     │                        │
          ┌──────────▼────────────────────────▼──────────┐
          │              ws-ctx-engine Core Engine            │
          │                                                │
          │  ┌───────────────┐  ┌───────────────────────┐ │
...
ws-ctx-engine search "user authentication flow" --agent-mode --limit 10
```
...
ws-ctx-engine search "auth" --agent-mode
ws-ctx-engine pack --agent-mode --format json
ws-ctx-engine index . --agent-mode
```

### 4.5 Agent Workflow (Expected)

```
Agent: ws-ctx-engine search "JWT refresh token logic" --agent-mode --limit 5
...
```bash
ws-ctx-engine pack --query "authentication" --format json --output context.json
```
...
```bash
ws-ctx-engine pack --query "authentication" --format md --output context.md
```
...
Entry point: `ws-ctx-engine mcp --workspace /path/to/repo`
...
```jsonc
{
  ...
  "recommendation": "Rebuild the index with: ws-ctx-engine index .",
...
```json
{"error": "TOOL_NOT_FOUND", "message": "ws-ctx-engine MCP server is read-only."}
```
...
```bash
ws-ctx-engine mcp --workspace /home/user/my-project
```
...
Secret scan results are cached in a persistent cache stored alongside the ws-ctx-engine index:
...
|---|---|
| `ws-ctx-engine search` (CLI) | **Off by default.** Human operator is accountable. Enable with `--secrets-scan`. |
| `ws-ctx-engine pack` (CLI) | **Off by default.** Enable with `--secrets-scan`. |
| `ws-ctx-engine mcp` (MCP server) | **Always on.** Cannot be disabled. MCP server may be queried by any agent. |
...
Configurable via `--rate-limit` flag or `.ws-ctx-engine/mcp_config.json`.
...
| Task | Owner | Notes |
|---|---|---|
| Implement `ws-ctx-engine search "<query>"` | Core | Uses existing FAISS index |
...
**Deliverable:** `ws-ctx-engine search "query" --agent-mode` returns clean NDJSON.

**Acceptance test:**
```bash
ws-ctx-engine search "authentication middleware" --agent-mode --limit 3 | jq '.path'
# → "src/auth/login.py"
...
**Deliverable:** `ws-ctx-engine pack --format json --secrets-scan` produces clean, parseable JSON.

**Acceptance test:**
```bash
ws-ctx-engine pack --query "auth" --format json | python3 -c "
import json, sys
...
**Goal:** Expose ws-ctx-engine's retrieval engine as a first-class MCP server.

**Week 7–8: Core MCP Infrastructure**

| Task | Notes |
|---|---|
| Create `ws_ctx_engine/mcp_server.py` | Python MCP SDK |
| Implement `ws-ctx-engine mcp --workspace PATH` entry point | |
...
|---|---|---|
| Should `get_file_context` return raw content or structured object? | **Structured object** with `path`, `language`, `line_count`, `content` (delimited), `dependencies`, `dependents`, `secrets_detected` | Raw content alone forces agent to re-derive metadata; structured response is ws-ctx-engine's differentiation |
...
| Should `include_dependencies` default to `true` or `false`? | **`true` (depth=1 only)** | Dependency graph is ws-ctx-engine's core differentiator; defaulting to off means agents never discover it |
...
├── cli/
│   ├── main.py
│   ├── search.py          ← NEW: ws-ctx-engine search command
