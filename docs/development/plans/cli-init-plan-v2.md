# wsctx-init — Enhancement Plan v2
...
|---|---|---|---|
| ① | `init_cli.py` broken path sau pip install | ✅ Đã fix | Di chuyển `scripts/` vào `src/ws_ctx_engine/`, sửa path |
| ② | Timeout khai báo nhưng không dùng | ✅ Đã fix | `timeout "$timeout_seconds" ws-ctx-engine index ...` |
| ③ | Hardcode macOS Python path | ✅ Đã fix | `_ensure_ws_ctx_engine_in_path()` dùng `python3 -m site` |
| ④ | `envsubst` nuốt `$VAR` trong code block | ✅ Đã fix | Scoped: `envsubst '${CTX_DATE} ${CTX_ENGINE_VERSION} ...'` |
...
**File:** `src/ws_ctx_engine/scripts/lib/core.sh`
...
```markdown
# ws-ctx-engine — Code Packaging   ← marker, bắt đầu delete
                                  ← blank line → SED DỪNG Ở ĐÂY
## What is ws-ctx-engine?           ← vẫn còn trong file sau remove!
```
...
```bash
_remove_section() {
  local file="$1" marker="${2:-ws-ctx-engine}"
  [[ -f "$file" ]] || return 0
  local tmp
  tmp="$(mktemp)"
  awk -v marker="^# ${marker}" '
...
```toml
ws_ctx_engine = ["py.typed", "scripts/**", "templates/**"]
```

`scripts/**` include cả `publish.sh` — đây là dev/release tool, không liên quan đến người dùng cuối. Ai install `ws-ctx-engine` qua pip sẽ có `publish.sh` trong `site-packages`.
...
# MANIFEST.in:
# prune src/ws_ctx_engine/scripts/publish.sh
```
...
Khi user chạy `wsctx-init` không có `--agents`, flow nên là:
...
```
ws-ctx-engine index .
ws-ctx-engine query "<query>"
ws-ctx-engine pack . --query "<topic>"
...
_discover_commands() {
  ...
  if command -v ws-ctx-engine &>/dev/null; then
  ...
  export CTX_CMD_QUERY='ws-ctx-engine '"${CMD_QUERY:=query}"' "<query>"'
...
_render_template() {
  ...
  export CTX_ENGINE_VERSION="$(ws-ctx-engine --version 2>/dev/null | head -1 || echo 'latest')"
...
# ...
install_ws_ctx_engine       # install trước
_discover_commands       # discover sau khi đã install
```

### 4.3 Benefit

Khi `ws-ctx-engine query` đổi thành `ws-ctx-engine search`:
...
```bash
# HIỆN TẠI — error messages từ ws-ctx-engine không phân biệt được
if timeout "$timeout_seconds" ws-ctx-engine index "$target" 2>&1; then

# FIX — giữ stderr riêng, chỉ redirect khi cần
if timeout "$timeout_seconds" ws-ctx-engine index "$target"; then
...
├── init.sh (_discover_commands sau install_ws_ctx_engine)
